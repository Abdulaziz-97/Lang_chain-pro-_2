    ...
    for day in range(days):
        for hour in range(24):
            forecast["hourly"].append({
                ...
                "temperature_c": random.uniform(10, 30),
                "condition": random.choice(["sunny", "partly_cloudy", "cloudy"]),
                "solar_irradiance": random.uniform(0, 1000),
                "humidity": random.uniform(0, 100),
                "wind_speed": random.uniform(0, 10)
            })
...
def get_electricity_prices(date: str = None) -> Dict[str, Any]:
    ...
    for hour in range(24):
        ...
        if is_peak:
            rate = base_price * random.uniform(1.2, 1.5)  # Peak pricing
            period = "peak"
            demand_charge = random.uniform(0.02, 0.05)
        else:
            rate = base_price * random.uniform(0.6, 0.8)  # Off-peak pricing
            period = "off_peak"
            demand_charge = 0
